...
from ac_jobs import ImmutableArtifactStore, JsonValue, RunRepository, RunSpec, RunStatus
from ac_proposer_reviewer.handler import ProposerReviewerHandler
from ac_proposer_reviewer.identity import derive_batch_run_id
...
from ac_proposer_reviewer.runner import BatchInputPayload, BatchRunner
...
def test_prepare_derives_run_id_and_read_request_validates_closed_spec(
    ...
    assert BATCH_SCHEMA_VERSION == "ac.proposer_reviewer.batch.v7"
    assert ProposerReviewerHandler.name == "ac.proposer_reviewer.batch.v4"
    runner = BatchRunner()
    request = _request()
...
def test_run_and_resume_keep_operational_options_out_of_semantic_input(
    ...
    from ac_proposer_reviewer import runner as runner_module
