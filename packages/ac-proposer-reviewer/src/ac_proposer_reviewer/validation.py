...
from ac_jobs import InvalidRunIdError, JsonValue, validate_simple_id
from ac_llm import AcLLMError, JsonOutput, LLMExecutionOptions, LLMInputArtifact
...
def _validate_loop(loop: LoopSpec, path: tuple[str | int, ...]) -> None:
    ...
    for worker_index, worker in enumerate((*loop.proposers, loop.reviewer)):
        ...
        except (AcLLMError, TypeError, ValueError) as exc:
            raise RequestValidationError(
