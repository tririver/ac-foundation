...
The adapter deliberately owns only the AC Foundation-side transport.  Provider
...
REQUEST_SCHEMA_VERSION = "ac.dsh-llm.request.v1"
EVENT_SCHEMA_VERSION = "ac.dsh-llm.event.v1"
...
class DshAdapter:
    ...
    def resume(
        ...
        return ProviderExecution(
            ...
            failure=ProviderFailure(
                "The DSH bridge does not expose native provider resume; AC Foundation must start a fresh generation.",
    ...
    def _call(
        ...
        if not token:
            return _failed(
                "The DSH AC Foundation bridge token is missing, empty, or not private.",
        ...
        try:
            ...
            failure = ProviderFailure(
                f"Could not reach the DSH AC Foundation bridge: {exc}",
                category=FailureCategory.TRANSPORT,
                retryable=True,
    ...
    def _paths_and_route(
        ...
        socket_path = self.socket_path or Path(
            source.get("AC_DSH_LLM_SOCKET")
            or source.get("DSH_AC_LLM_SOCKET")
            or (Path.home() / ".dsh" / "runtime" / "ac-llm.sock")
        )
        token_path = self.token_path or Path(
            source.get("AC_DSH_LLM_TOKEN_FILE")
            or source.get("DSH_AC_LLM_TOKEN_FILE")
            or f"{socket_path}.token"
        )
        provider_route = self.provider_route or source.get(
            "AC_DSH_PROVIDER", DEFAULT_PROVIDER_ROUTE
        )
...
def _workspace_bridge_prompt(workspace: Path) -> str:
    ...
    try:
        ...
        raise ProviderFailure(
            "The DSH provider could not read AC Foundation's workspace control document.",
    ...
    bridge_prompt = (
        "You are AC Foundation's native text-generation backend. The JSON document below "
    ...
    if len(bridge_prompt.encode("utf-8")) > _MAX_BRIDGE_PROMPT_BYTES:
        raise ProviderFailure(
            "The AC Foundation workspace is too large for the DSH bridge request.",
