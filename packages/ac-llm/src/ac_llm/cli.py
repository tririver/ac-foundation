"""Machine-oriented CLI using the shared ac-jobs command protocol."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    generate.add_argument(
        ...
        help="host authority attestation (default: unknown, so AC Foundation uses host turns)",
    )
    ...
    doctor = commands.add_parser(
        ...
        description="Check whether an AC Foundation LLM provider is configured and executable.",
    )
    doctor.add_argument(
        "--provider",
...
def _failure(
    ...
    *,
    ...
    if isinstance(exc, AcLLMError):
        ...
        code = "local_io_error"
        details = {}
    elif isinstance(exc, AcJobsError):
        code = {
...
def _help_command(arguments: list[str]) -> str:
    ...
    return " ".join(
        part for part in ("ac-llm", command, "--help") if part is not None
    )
