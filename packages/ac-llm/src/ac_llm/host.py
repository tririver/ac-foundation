...
These values describe the process in which an AC consumer is running. They are runtime
...
class AcRuntimeEnvironment:
    """The small, immutable AC environment surface inherited by a provider."""

    values: Mapping[str, str | None] = field(default_factory=dict)

    def __post_init__(self) -> None:
        raw = dict(self.values)
        if set(raw) != set(_AC_ENVIRONMENT_KEYS):
            raise InvalidRequestError(
                "AC runtime environment must contain exactly AC_HOME, "
                "AC_RUNTIME_HOME, AC_DOCUMENT_CACHE, and PATH."
            )
        if any(value is not None and not isinstance(value, str) for value in raw.values()):
            raise InvalidRequestError("AC runtime environment values must be strings or null.")
...
HOST_TURN_SCHEMA_VERSION = "ac.llm.host_turn.v1"
HOST_CONTINUATION_SCHEMA_VERSION = "ac.llm.host_continuation.v1"
...
def host_response_document(response: HostResponse) -> dict[str, Any]:
    return {
        "schema_version": "ac.llm.host_response.v1",
...
def decode_host_response(value: Any) -> HostResponse:
    ...
    if value["schema_version"] != "ac.llm.host_response.v1":
