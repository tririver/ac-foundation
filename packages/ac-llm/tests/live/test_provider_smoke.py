...
pytestmark = pytest.mark.skipif(
    os.environ.get("AC_RUN_LIVE_PROVIDER_SMOKE") != "1"
    or os.environ.get("AC_RUN_NET_TESTS") != "1",
    reason=(
        "live provider smoke requires AC_RUN_LIVE_PROVIDER_SMOKE=1 and "
        "AC_RUN_NET_TESTS=1"
    ),
)
...
def test_live_provider_strict_json_continuation_and_local_replay() -> None:
    root_value = os.environ.get("AC_LLM_SMOKE_ROOT")
    if not root_value:
        pytest.fail("AC_LLM_SMOKE_ROOT must name an ignored run directory")
    root = Path(root_value).resolve()
    repository_root = Path(__file__).resolve().parents[4]
    smoke_parent = repository_root / "local" / "net-tests"
    if not root.is_relative_to(smoke_parent):
        pytest.fail("AC_LLM_SMOKE_ROOT must be located under local/net-tests/")
    if root.exists() and (not root.is_dir() or any(root.iterdir())):
        pytest.fail("AC_LLM_SMOKE_ROOT must not exist or must be an empty directory")
    ...
    if ignored.returncode != 0:
        pytest.fail("AC_LLM_SMOKE_ROOT must be covered by repository git-ignore")
    root.mkdir(parents=True, exist_ok=True)
    provider = os.environ.get("AC_LLM_SMOKE_PROVIDER", "auto")
    repository = RunRepository(root)
    parent = repository.create(
        RunSpec(
            "live-provider-smoke",
            "ac.llm.live_smoke.v1",
            {"provider": provider},
        )
    )
    context = RunContext(
