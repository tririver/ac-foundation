...
from ac_llm import ProviderRequest, ProviderTerminalKind
from ac_llm.errors import FailureCategory, ProviderFailure
from ac_llm.providers.dsh import (
    DshAdapter,
...
def test_dsh_adapter_consumes_authenticated_ndjson_stream(tmp_path: Path) -> None:
    ...
    socket_path = tmp_path / "ac-llm.sock"
    token_path = tmp_path / "ac-llm.token"
...
def test_dsh_adapter_rejects_unknown_event_schema() -> None:
    raw = json.dumps(
        {"schema_version": "ac.dsh-llm.event.v999", "type": "started"}
    ).encode("utf-8")
