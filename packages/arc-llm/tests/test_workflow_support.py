...
from arc_llm import workflow_support
...
def test_semantic_retry_is_deterministic_and_namespace_owned() -> None:
    ...
    first = semantic_retry_request(
        request,
        identity_schema_version="arc.example.semantic_retry.v1",
        validator_contract="arc.example.output.v1",
    ...
    changed_namespace = semantic_retry_request(
        request,
        identity_schema_version="arc.other.semantic_retry.v1",
        validator_contract="arc.example.output.v1",
        feedback="missing field",
    )
    ...
    assert "Validator contract: arc.example.output.v1" in first.prompt
