...
from arc_llm.memory import read_memory_availability
...
def test_cgroup_v2_headroom_wins_when_more_constrained(tmp_path: Path) -> None:
    ...
    _write(proc / "self" / "cgroup", "0::/arc.slice\n")
    _write(cgroup / "arc.slice" / "memory.max", "1000\n")
    _write(cgroup / "arc.slice" / "memory.current", "950\n")
