...
def test_command_codec_is_closed_and_enforces_invariants():
    ...
    assert document["schema_version"] == "ac.command_result.v2"
    ...
    with pytest.raises(CorruptStateError):
        decode_command_result(
            {**document, "schema_version": "ac.command_result.v1"}
        )
...
def test_run_snapshot_v3_reads_v2_epoch_zero_and_rejects_v1(tmp_path):
    ...
    assert document["schema_version"] == "ac.jobs.run_snapshot.v3"
    ...
    document["schema_version"] = "ac.jobs.run_snapshot.v1"
    path.write_text(json.dumps(document))
