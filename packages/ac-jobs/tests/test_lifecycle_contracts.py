...
import ac_jobs.engine as engine_module
import ac_jobs.groups as group_storage
import ac_jobs.lease as lease_module
...
def test_group_documents_use_v2_and_reject_legacy_v1(tmp_path):
    ...
    assert state["schema_version"] == "ac.jobs.group.v2"
    assert unit["schema_version"] == "ac.jobs.group_unit.v2"

    state["schema_version"] = "ac.jobs.group.v1"
    state_path.write_text(json.dumps(state))
    with pytest.raises(UnsupportedSchemaError):
        repository.inspect_group("run-1", "group")

    state["schema_version"] = "ac.jobs.group.v2"
    state_path.write_text(json.dumps(state))
    unit["schema_version"] = "ac.jobs.group_unit.v1"
...
def test_legacy_v2_failed_run_lazy_materializes_working_state_on_resume(tmp_path):
    ...
    document["schema_version"] = "ac.jobs.run_snapshot.v2"
