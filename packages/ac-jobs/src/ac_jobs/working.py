...
class WorkingState:
    ...
    def materialize(self, spec: RunSpec, *, include_legacy_artifacts: bool = False) -> None:
        ...
        if not self.index_path.exists():
            atomic_write_json(
                self.index_path,
                {
                    "schema_version": "ac.jobs.working_index.v1",
                    "recovery_epoch": 0,
                    "updated_at": utc_now(),
                    "snapshot_path": None,
    ...
    def record_error(self, error: RunError, *, attempt: int, recovery_epoch: int) -> None:
        atomic_write_json(
            ...
            {
                "schema_version": "ac.jobs.working_error.v1",
                "attempt": attempt,
                "recovery_epoch": recovery_epoch,
                "recorded_at": utc_now(),
                "error": {
...
class EditableArtifactStore:
    ...
    def scoped(
        ...
        if combined.startswith("llm/tasks/"):
            # Provider task protocol records are AC Foundation-managed execution state,
