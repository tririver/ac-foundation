...
def encode_command_result(value: CommandResult) -> dict[str, JsonValue]:
    ...
    return {
        "schema_version": "ac.command_result.v2",
...
def decode_command_result(document: Mapping[str, JsonValue]) -> CommandResult:
    ...
    if document["schema_version"] != "ac.command_result.v2":
...
def decode_progress_event(document: Mapping[str, JsonValue]) -> ProgressEvent:
    ...
    if document["schema_version"] != "ac.progress_event.v1":
...
def snapshot_data(snapshot: RunSnapshot) -> dict[str, JsonValue]:
    data: dict[str, JsonValue] = {
        ...
        "can_resume": snapshot.status is not RunStatus.SUCCEEDED,
        "working_state_path": "working",
        "working_state": {
            "root": "working",
            "semantic_input": "working/semantic-input.json",
            "index": "working/index.json",
            "artifacts": "working/artifacts",
            "candidates": "working/candidates",
            "last_error": "working/last-error.json",
        },
