...
REGISTRY_SCHEMA_VERSION = "arc.document.operation_registry.v1"
_STRING = {"type": "string"}
...
def _service(cache_root: str | None) -> ArcDocumentService:
    return ArcDocumentService(cache_root=cache_root)
...
def _spec(
    ...
    *,
    ...
    return operation_spec(
        package_name="arc-document",
        schema_namespace="arc.document",
        name=name,
        schema=schema,
        callable=callable,
...
def dispatch_operation(operation: str, parameters: Mapping[str, Any]) -> Any:
    ...
    if spec is None:
        raise OperationRequestError(
            "operation_not_found",
            f"unknown arc-document operation: {operation}",
        )
