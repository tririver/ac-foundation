...
from ac_jobs import ArtifactSourceRef, RunContext, StoppedError
from ac_llm import (
    JsonOutput,
    LLMStopped,
    LLMCompleted,
...
VISUAL_PAGE_REVIEW_SCHEMA = "ac.document.visual_page_review.v1"
VISUAL_PAGE_TERMINAL_SCHEMA = "ac.document.visual_page_terminal.v1"
...
class PdftoppmFullPageRenderer:
    ...
    def render(self, pdf_bytes: bytes) -> tuple[RenderedPDFPage, ...]:
        ...
        with tempfile.TemporaryDirectory(prefix="ac-document-pdf-render-") as directory:
            root = Path(directory)
    ...
    def render_page(self, pdf_bytes: bytes, page_number: int) -> RenderedPDFPage:
        ...
        with tempfile.TemporaryDirectory(prefix="ac-document-pdf-page-") as directory:
            root = Path(directory)
            source = root / "source.pdf"
            target = root / "page"
            source.write_bytes(pdf_bytes)
...
def _math_manifest(spans: tuple[MathSpan, ...]) -> dict[str, Any]:
    return {
        "schema_version": "ac.document.math_span_manifest.v1",
        "spans": [
            {
